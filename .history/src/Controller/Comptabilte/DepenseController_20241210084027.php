...
class DepenseController extends BaseController
{
    ...
    public function edit(Request $request, Depense $depense, EntityManagerInterface $entityManager,LignedepenseRepository $lignedepenseRepository ,FormError $formError): Response
    {
        $validationGroups = ['Default', 'FileRequired', 'oui'];
        ...
        // }
        $typedepense = $typedepenseRepository->findAll();
        // Ajout des lignes de dépenses en fonction des types de dépenses
        foreach ($typedepense as $key => $value) {
            // Vérifie si une ligne de dépense existe déjà pour ce type
            $ligneExistante = $depense->getLignedepenses()->filter(function ($ligne) use ($value) {
                return $ligne->getTypedepense() === $value;
            });

            // Si la ligne n'existe pas, créez une nouvelle ligne de dépense
