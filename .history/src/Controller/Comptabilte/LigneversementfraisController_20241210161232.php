...
class LigneversementfraisController extends BaseController
{
 ...
 #[Route('/liste/paiement/{idR}', name: 'app_comptabilte_ligneversementfraiss_index', methods: ['GET', 'POST'])]
    public function indexListeVersement(Request $request, DataTableFactory $dataTableFactory, int $id): Response
    {
    ...
    public function new(Request $request, Compte $compte,Ligneversementfrais $ligneversementfrais,LigneversementfraisRepository $ligneversementfraisRepository,  EntityManagerInterface $entityManager, FormError $formError): Response
    {
        ...
        if ($form->isSubmitted()) {
            ...
            if ($form->isValid()) {
                ...
                $url = [
                    'url' => $this->generateUrl('app_comptabilte_ligneversementfraiss_index', [
                        'id' => $compte->getId()
                    ]),
                    'tab' => '#module0',
                    'current' => '#module0'
    ...
    public function edit(
        ...
        FormError $formError,
        LigneversementfraisRepository $ligneversementfraisRepository,
    ): Response {
        ...
        $oldMontant = (int)$ligneversementfrais->getMontant();
        ...
        if ($form->isSubmitted()) {
            ...
            if ($form->isValid()) {
                ...
                //     $service->paiementInscriptionEdit($ligneversementfrai);
                // }

                $url = [
                    'url' => $this->generateUrl('app_inscription_liste_versement_index', [
                        'id' => $ligneversementfrais->getCompte()->getId()
                    ]),
                    'tab' => '#module1',
                    'current' => '#module1'
                ];
